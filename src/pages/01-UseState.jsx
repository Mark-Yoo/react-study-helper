  ...
  // 여기에 코드를 작성하세요
        ...
        <div className="problem-workspace">
          <div className="result-area">
            <h3>카운트: {count}</h3>
            <button
              onClick={() => (mode === 'view' || isCorrect[1]) && setCount(count + 1)}
...
onClick={() => setCount(count + 1)}`}</pre>
      ...
      <section className="example-section problem-section">
        ...
        <div className="problem-description">
          <h3>📌 요구사항:</h3>
          <ul>
            <li>입력 필드에 타이핑한 내용이 실시간으로 아래에 표시되어야 합니다</li>
...
/>`}</pre>
        ...
        <div className="problem-workspace">
          <div className="result-area">
            <input
              type="text"
              placeholder={mode === 'practice' && !isCorrect[2] ? '🔒 코드를 확인하여 활성화' : '여기에 입력하세요'}
              value={inputValue}
              onChange={(e) => (mode === 'view' || isCorrect[2]) && setInputValue(e.target.value)}
...
onChange={(e) => setInputValue(e.target.value)}`}</pre>
      ...
      <section className="example-section problem-section">
        ...
        {mode === 'practice' && (
          <div className="code-editor-section">
            <h4>💻 코드 작성 영역:</h4>
            <p className="code-hint">체크박스의 onChange 핸들러를 작성하세요:</p>
            <div className="code-template">
          ...
          <div className="result-area">
            <label>
              <input
                type="checkbox"
                checked={isChecked}
                onChange={() => (mode === 'view' || isCorrect[3]) && setIsChecked(!isChecked)}
        ...
        <div className="problem-workspace">
          <div className="result-area">
            <div className="button-group">
              <button
                onClick={() => (mode === 'view' || isCorrect[4]) && setSelectedColor('red')}
                disabled={mode === 'practice' && !isCorrect[4]}
              >
                빨강
              </button>
              <button
                onClick={() => (mode === 'view' || isCorrect[4]) && setSelectedColor('green')}
                disabled={mode === 'practice' && !isCorrect[4]}
              >
                초록
              </button>
              <button
                onClick={() => (mode === 'view' || isCorrect[4]) && setSelectedColor('blue')}
                disabled={mode === 'practice' && !isCorrect[4]}
              >
                파랑
              </button>
...
onClick={() => setSelectedColor('blue')}`}</pre>
      ...
      <section className="example-section problem-section">
        ...
        <div className="problem-description">
          <h3>📌 요구사항:</h3>
          <ul>
            <li>입력 필드에 할일을 입력하고 추가 버튼을 클릭하면 리스트에 추가되어야 합니다</li>
            <li>각 항목 옆의 삭제 버튼을 클릭하면 해당 항목이 제거되어야 합니다</li>
        ...
        <div className="problem-workspace">
          <div className="result-area">
            <div className="todo-input">
              <input
                type="text"
                placeholder={mode === 'practice' && !isCorrect[5] ? '🔒 코드를 확인하여 활성화' : '할일 입력'}
                value={newItem}
                onChange={(e) => (mode === 'view' || isCorrect[5]) && setNewItem(e.target.value)}
                onKeyPress={(e) => {
                  if ((mode === 'view' || isCorrect[5]) && e.key === 'Enter' && newItem.trim()) {
                    setItems([...items, { id: Date.now(), text: newItem }])
                    setNewItem('')
                  }
                }}
            ...
            <ul className="todo-list">
              {items.map(item => (
                <li key={item.id}>
                  <span>{item.text}</span>
                  <button
                    onClick={() => (mode === 'view' || isCorrect[5]) && setItems(items.filter(i => i.id !== item.id))}
                    disabled={mode === 'practice' && !isCorrect[5]}
                  >
                    삭제
                  </button>
      ...
      <section className="example-section problem-section">
        ...
        <div className="problem-description">
          ...
          <ul>
            ...
            <li>각 입력 필드가 변경될 때마다 user 객체의 해당 속성만 업데이트되어야 합니다</li>
...
/>`}</pre>
          ...
          <div className="result-area">
            <div className="form-group">
              ...
              <input
                type="text"
                value={user.name}
                onChange={(e) => (mode === 'view' || isCorrect[6]) && setUser({ ...user, name: e.target.value })}
                disabled={mode === 'practice' && !isCorrect[6]}
                placeholder={mode === 'practice' && !isCorrect[6] ? '🔒 코드를 확인하여 활성화' : ''}
              />
            </div>
            <div className="form-group">
              <label>나이:</label>
              <input
                type="number"
                value={user.age}
                onChange={(e) => (mode === 'view' || isCorrect[6]) && setUser({ ...user, age: e.target.value })}
                disabled={mode === 'practice' && !isCorrect[6]}
                placeholder={mode === 'practice' && !isCorrect[6] ? '🔒 코드를 확인하여 활성화' : ''}
              />
            </div>
        ...
        {showHint[6] && (
          <div className="hint-box">
            <strong>💡 힌트:</strong> 객체 spread 연산자 {...user}를 사용하여
            기존 속성을 복사하고, 변경할 속성만 덮어쓰세요.
          </div>
        )}
  ...
  // -5 버튼: 여기에 코드를 작성하세요
        ...
        <div className="problem-workspace">
          <div className="result-area">
            ...
            <div className="button-group">
              <button
                onClick={() => (mode === 'view' || isCorrect[7]) && setComplexCount(prev => prev + 5)}
                disabled={mode === 'practice' && !isCorrect[7]}
              >
                +5
              </button>
              <button
                onClick={() => (mode === 'view' || isCorrect[7]) && setComplexCount(prev => prev + 10)}
                disabled={mode === 'practice' && !isCorrect[7]}
              >
                +10
              </button>
              ...
              <button
                onClick={() => (mode === 'view' || isCorrect[7]) && setComplexCount(0)}
                disabled={mode === 'practice' && !isCorrect[7]}
              >
                리셋
              </button>
